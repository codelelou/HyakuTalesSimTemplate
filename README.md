...
Unreal Engineには標準でコントローラー振動機能（ForceFeedback）がありますが、コントローラー振動を一括で処理する共通システムを用意しています。  
理由は、振動中にゲームの一時停止すると一時停止を解除するまで振動しっぱなしになってしまう現象を解消のためです。特に気にしないのであれば標準のコントローラー振動機能を使っても問題ありません。  

蝋燭（/Content/HyakuTales/Blueprint/Actor/Candle/HT_BP_Actor_Candle_Base.uasset）の消灯時にサンプルとしてわずかですがコントローラー振動を行っています（手で仰ぐ・息を吹きかけるアクションをイメージ）。  
...
ただゲーム開発初心者の内は開発の負担を少しでも減らすために、割り切ってコントローラー振動を実装しないという判断も有りかとは思います。  
特にPCゲームであればキーボード&マウスでプレイするプレイヤーも多く、ゲーム体験に影響するケースは限定的とも考えられます。  
